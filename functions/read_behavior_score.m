function variables = read_behavior_score(parameters)
    ...
    for v = 1 : numel(varsToCheck)
        ...
        if v == 1 % then it's the outcome measure, the main value... save it.
            ...
            variables.covariatestable = table;
    ...
    % we now use ismissing and concat the variables as tables -- to support categorical variables
    remove_idx = find(any(ismissing([table(variables.one_score) variables.covariatestable]),2));
...
%     end
    if ~isempty(variables.covariatestable)
        variables.covariatestable(remove_idx,:) = []; % remove whole rows.
    end
